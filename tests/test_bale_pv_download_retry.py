...
from app.connectors.bale_pv_connector import BalePvConnector, BalePvInstanceRuntime
...
async def test_normalize_raises_when_all_attachment_downloads_failed():
    ...
    with patch("app.runtime_registry.get_runtime", return_value=runtime):
...
def _connector_with_runtime(tmp_path, monkeypatch, get_responses):
    ...
    monkeypatch.setattr(
        "app.connectors.bale_pv_connector.FILE_DOWNLOAD_RETRY_BACKOFF_SECONDS", 0
    )
