...
from wootify.application.messaging.payload_parser import MessagePayloadParser
...
from wootify.application.messaging.payload_sanitizer import sanitize_payload
...
class BridgeService:
    ...
    def __init__(self) -> None:
        ...
        self._notification_policy = NotificationPolicy(self._status_notify_recent)
        self._bale_pv_sync_workflow = BalePvSyncWorkflow(self)
    ...
    async def sync_bale_pv_contacts(
        ...
        """Synchronize Bale contacts through the dedicated workflow object."""
        return await self._bale_pv_sync_workflow.sync_contacts(db, instance_key, runtime)
    ...
    async def sync_bale_dialogs_to_chatwoot(
        ...
        *,
        ...
        """Synchronize Bale dialogs through the dedicated workflow object."""
        return await self._bale_pv_sync_workflow.sync_dialogs(
            db, instance_key, runtime, load_history=load_history, history_limit=history_limit
        )
    ...
    def _is_chatwoot_status_event(payload: dict[str, Any], event_name: str) -> bool:
        ...
        return NotificationPolicy.status_event(payload, event_name)
    ...
    def _status_notification_text(
        ...
        *,
        ...
        """Render a configured status notification."""
        return NotificationPolicy.text(
            status_name, operator_name=operator_name, platform_metadata=platform_metadata
        )
    ...
    def _status_notification_enabled(platform_metadata: Optional[dict[str, Any]]) -> bool:
        ...
        return NotificationPolicy.enabled(platform_metadata)
    ...
    def _status_notification_templates(platform_metadata: Optional[dict[str, Any]]) -> dict[str, Optional[str]]:
        ...
        return NotificationPolicy.templates(platform_metadata)

    @staticmethod
    def _string_or_none(value: Any, *, fallback: str = '') -> Optional[str]:
        """Internal helper to string or none."""
        return NotificationPolicy.string_or_none(value, fallback=fallback)
    ...
    def _is_duplicate_status_notification(self, instance_key: str, conversation_id: str, status_name: str) -> bool:
        ...
        return self._notification_policy.is_duplicate(instance_key, conversation_id, status_name)
    ...
    def _mark_status_notification(self, instance_key: str, conversation_id: str, status_name: str) -> None:
        ...
        self._notification_policy.mark(instance_key, conversation_id, status_name)
    ...
    def _split_prefixed_source_id(value: Optional[str]) -> tuple[Optional[str], Optional[str]]:
        """Split a platform-prefixed source id."""
        return DestinationResolver.split_prefixed_source_id(value)
    ...
    def _extract_destination(
        ...
        *,
        ...
        """Extract a destination respecting platform source prefixes."""
        expected_prefix = self._source_prefix(platform_key) if platform_key else None
        return DestinationResolver.extract_destination(
            payload, expected_prefix=expected_prefix, known_prefixes=connector_registry.all_prefixes()
        )
    ...
    def _normalize_attachment_for_chatwoot(
        ...
        *,
        ...
        """Normalize attachment metadata using the application policy."""
        return MediaNormalizer.for_chatwoot(filename=filename, content_type=content_type, content=content)
    ...
    def _normalize_filename_for_platform(
        ...
        return MediaNormalizer.for_platform(filename, content_type, url)

    @staticmethod
    def _preferred_extension_for_content_type(content_type: str) -> Optional[str]:
        """Return the preferred media extension."""
        return MediaNormalizer.preferred_extension(content_type)

    @staticmethod
    def _guess_content_type_from_bytes(content: bytes) -> Optional[str]:
        """Guess a content type from file signatures."""
        return MediaNormalizer.guess_content_type(content)
    ...
    def _choose_destination_chat_id(
        ...
        """Choose the most trustworthy platform destination."""
        return DestinationResolver.choose_chat_id(mapped_destination, extracted_destination)

    @staticmethod
    def _looks_like_uuid(value: str) -> bool:
        """Return whether a value is a Chatwoot UUID."""
        return DestinationResolver.looks_like_uuid(value)

    @staticmethod
    def _extract_attachments(payload: dict[str, Any]) -> list[dict[str, Any]]:
        """Extract direct or nested Chatwoot attachments."""
        return MessagePayloadParser.extract_attachments(payload)

    @staticmethod
    def _extract_chatwoot_message_text(payload: dict[str, Any]) -> str:
        """Extract message text from the supported Chatwoot shapes."""
        return MessagePayloadParser.extract_message_text(payload)

    @staticmethod
    def _normalize_chatwoot_message_type(value: Any) -> str:
        """Normalize Chatwoot's numeric or string message type."""
        return MessagePayloadParser.normalize_message_type(value)

    @staticmethod
    def _is_forwardable_chatwoot_message(payload: dict[str, Any], event_name: str) -> bool:
        """Return whether a webhook message should be forwarded."""
        return MessagePayloadParser.is_forwardable_message(payload, event_name)
    ...
    def _is_chatwoot_message_deleted(payload: dict[str, Any]) -> bool:
        ...
        """
        return MessagePayloadParser.is_message_deleted(payload)
    ...
    def _extract_chatwoot_conversation_id(payload: dict[str, Any]) -> Optional[str]:
        """Extract the Chatwoot conversation id."""
        return MessagePayloadParser.extract_conversation_id(payload)

    @staticmethod
    def _extract_chatwoot_message_id(payload: dict[str, Any]) -> Optional[str]:
        """Extract the Chatwoot message id."""
        return MessagePayloadParser.extract_message_id(payload)

    @staticmethod
    def _extract_parent_chatwoot_message_id(payload: dict[str, Any]) -> Optional[str]:
        """Extract an optional reply-to message id."""
        return MessagePayloadParser.extract_parent_message_id(payload)

    @staticmethod
    def _extract_contact_id(payload: dict[str, Any]) -> Optional[str]:
        """Extract the Chatwoot contact id."""
        return MessagePayloadParser.extract_contact_id(payload)

    @staticmethod
    def _normalize_phone_number(value: Any) -> Optional[str]:
        """Normalize phone values into Chatwoot-compatible text."""
        return MessagePayloadParser.normalize_phone_number(value)

    @staticmethod
    def _extract_phone_from_shared_text(value: Any) -> Optional[str]:
        """Extract a phone number embedded in a shared-contact message."""
        return MessagePayloadParser.extract_phone_from_shared_text(value)

    @staticmethod
    def _extract_contact_payload(value: Any) -> dict[str, Any]:
        """Unwrap a Chatwoot contact response."""
        return MessagePayloadParser.extract_contact_payload(value)
    ...
    def _build_contact_update_payload(
        ...
        *,
        ...
        """Build a contact update preserving existing optional fields."""
        return MessagePayloadParser.build_contact_update_payload(
            current_contact, normalized_phone, fallback_name=fallback_name
        )

    @staticmethod
    def _to_plus_phone_candidate(phone: Optional[str]) -> Optional[str]:
        """Return a plus-prefixed search candidate when appropriate."""
        return MessagePayloadParser.to_plus_phone_candidate(phone)
