...
from wootify.plugins.bale.novin_sms_client import NovinSmsClient
...
from wootify.infrastructure.persistence.repositories.enterprise_pending_message_repository import (
    EnterprisePendingMessageRepository,
)
from wootify.application.enterprise.documents import EnterpriseDocumentService
from wootify.application.enterprise.gre import EnterpriseGreValidator, GreValidationError
...
from wootify.infrastructure.observability.logging import log_sms_to_file
...
_BALE_ROUTE_POLICY = EnterpriseRoutePolicy(
    static_routes=ROUTE_CONFIG,
    text_key_map={
        "waiting": "waiting_text_key",
        "accepted": "accepted_text_key",
        "unread": "unread_text_key",
    },
)
_BALE_MENU_CONFIG = EnterpriseMenuConfig()
_BALE_SESSION_POLICY = EnterpriseSessionTransitionPolicy(
    non_live_states=frozenset(
        {
            EnterpriseUserState.awaiting_phone_input,
            EnterpriseUserState.eligible_root,
            EnterpriseUserState.ineligible_root,
            EnterpriseUserState.manual_group_menu,
            EnterpriseUserState.manual_menu,
            EnterpriseUserState.address_menu,
        }
    ),
    route_by_state={
        EnterpriseUserState.live_customer_service: ROUTE_CUSTOMER_SERVICE,
        EnterpriseUserState.live_sales: ROUTE_SALES,
    },
...
class EnterpriseBaleService:
    ...
    def _set_user_state(
        ...
        _BALE_SESSION_POLICY.set_user_state(self._users, db, user, state)
    ...
    def _require_route(route_key: str) -> dict[str, Any]:
        ...
        return _BALE_ROUTE_POLICY.require(None, route_key)
    ...
    def _route_text(
        ...
        return _BALE_ROUTE_POLICY.text(platform_metadata, route_key, kind)
    ...
    def _message_text(
        ...
        return _BALE_MENU_CONFIG.message(platform_metadata, field_name, default)
    ...
    def _not_configured_text(platform_metadata: Optional[dict[str, Any]]) -> str:
        ...
        return _BALE_MENU_CONFIG.not_configured(platform_metadata, NOT_CONFIGURED_TEXT)
