...
from wootify.infrastructure.persistence.repositories.enterprise_telegram_pending_message_repository import (
    EnterpriseTelegramPendingMessageRepository,
)
from wootify.infrastructure.persistence.repositories.enterprise_telegram_session_repository import (
    EnterpriseTelegramSessionRepository,
)
from wootify.infrastructure.persistence.repositories.enterprise_telegram_user_repository import (
    EnterpriseTelegramUserRepository,
)
from wootify.application.enterprise.documents import EnterpriseDocumentService
...
_TELEGRAM_ROUTE_POLICY = EnterpriseRoutePolicy()
_TELEGRAM_MENU_CONFIG = EnterpriseMenuConfig()
_TELEGRAM_SESSION_POLICY = EnterpriseSessionTransitionPolicy(
    non_live_states=frozenset(
        {STATE_ROOT, STATE_MANUAL_GROUP_MENU, STATE_MANUAL_MENU, STATE_ADDRESS_MENU}
    ),
    closed_status=EnterpriseSessionStatus.closed_by_user,
)
...
class EnterpriseTelegramService:
    ...
    async def _handle_root_actions(
        ...
        route = _TELEGRAM_ROUTE_POLICY.match_display_name(cfg, normalized)
    ...
    def _leave_live_session_if_needed(
        ...
        session = _TELEGRAM_SESSION_POLICY.active_session(self._sessions, db, user)
        if not session:
            return
        _TELEGRAM_SESSION_POLICY.close_session(self._sessions, db, session)

    def _active_live_session(
        self, db: Session, user: EnterpriseTelegramUser
    ) -> Optional[EnterpriseTelegramSession]:
        """Resolve the live session matching the user's current state (route_key)."""
        return _TELEGRAM_SESSION_POLICY.active_session(self._sessions, db, user)

    def _mark_user_present(self, db: Session, session: EnterpriseTelegramSession) -> None:
        """Mark the user as present in a live session."""
        _TELEGRAM_SESSION_POLICY.mark_user_present(self._sessions, db, session)
    ...
    def _is_live_state(self, state: str) -> bool:
        ...
        return _TELEGRAM_SESSION_POLICY.is_live_state(state)
    ...
    def _set_user_state(
        ...
        _TELEGRAM_SESSION_POLICY.set_user_state(
            self._users, db, user, str(state or STATE_ROOT)
        )
    ...
    def _require_route(
        ...
        return _TELEGRAM_ROUTE_POLICY.require(platform_metadata, route_key)

    def _route_text(
        self, platform_metadata: dict[str, Any], route_key: str, kind: str
    ) -> Optional[str]:
        """Resolve a route-specific configured text."""
        return _TELEGRAM_ROUTE_POLICY.text(platform_metadata, route_key, kind)
    ...
    def _route_config(
        ...
        return _TELEGRAM_ROUTE_POLICY.get(platform_metadata, route_key)
    ...
    def _message_text(
        ...
        return _TELEGRAM_MENU_CONFIG.message(platform_metadata, field_name, default)
    ...
    def _label(
        ...
        return _TELEGRAM_MENU_CONFIG.label(platform_metadata, field_name, default)
    ...
    def _not_configured_text(platform_metadata: Optional[dict[str, Any]]) -> str:
        ...
        return _TELEGRAM_MENU_CONFIG.not_configured(
            platform_metadata, NOT_CONFIGURED_TEXT
        )
    ...
    def _normalize_content_type(
        *, filename: str, content_type: Optional[str], content: bytes
        ...
        return EnterpriseChatwootPayloadPolicy.normalize_content_type(
    ...
    def _is_forwardable_chatwoot_message(
        ...
        return EnterpriseChatwootPayloadPolicy.is_forwardable(payload, event_name)
    ...
    def _normalize_chatwoot_message_type(value: Any) -> str:
        ...
        return EnterpriseChatwootPayloadPolicy.normalize_message_type(value)
