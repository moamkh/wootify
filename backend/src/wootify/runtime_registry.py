...
def _build_adapter(platform_type: str, instance_key: str, config: Dict[str, Any]) -> BasePlatformAdapter:
    plugin = platform_plugin_registry.get(platform_type)
    try:
        return plugin.build_adapter(instance_key, config)
    except ValueError as exc:
        raise ValueError(f"Unsupported platform_type for adapter registry: {platform_type}") from exc
