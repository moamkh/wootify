...
def test_configure_logging_adds_rotating_file_handler(tmp_path):
    ...
    with patch("wootify.logging_config.settings.LOG_FILE_PATH", str(log_file)):
        with patch("wootify.logging_config.settings.LOG_FILE_MAX_BYTES", 1024):
            with patch("wootify.logging_config.settings.LOG_FILE_BACKUP_COUNT", 2):
...
def test_configure_logging_relative_path_resolves_to_repo_root(tmp_path):
    ...
    from wootify.config import repo_root

    with patch("wootify.logging_config.settings.LOG_FILE_PATH", "sub_dir/test_backend.log"):
        root = logging.getLogger()
        original_handlers = list(root.handlers)
        for handler in original_handlers:
            root.removeHandler(handler)
